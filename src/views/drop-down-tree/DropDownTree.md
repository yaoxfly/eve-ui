...
> 在数据回填时，如果数据无法回填,要特别检查下`v-model`绑定的值 `value` 的赋值是否在树数据`data`赋值的下面，如果是懒加载，回填只有`id`时需要在`$nextTick`中赋值`option`，如果无论如何回填还是困难，请在异步操作(`ajax`)里重新刷新组件，可用`:key`或`v-if`来刷新组件。
